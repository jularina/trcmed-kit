...
def figure_4a(times, obs, meals, gpconv, gplfm, path):
    ...
    gplfm_baseline, gplfm_carbs, gplfm_fat, gplfm_glucose = gplfm['baseline'], gplfm['carbs'], gplfm['fat'], gplfm[
        'fitted_glucose']
    gpconv_baseline_var, gpconv_wfat_var, gpconv_wofat_var = gpconv['baseline_var'], gpconv['fitted_glucose_var'], \
        gpconv[
            'fitted_glucose_wofat_var']
    gplfm_baseline_var, gplfm_carbs_var, gplfm_fat_var, gplfm_glucose_var = gplfm['baseline_var'], gplfm['carbs_var'], \
        gplfm['fat_var'], gplfm[
        'fitted_glucose_var']
...
def figure_4b(times, meals, gpconv, gplfm, path):
    ...
    def plot_meal_bars(ax):
        ...
        ax.text(meals[:, 0], meals[:, 1] / 2, f"carbs.\n{meals[0, 1]:.1f}", ha='center', va='center',
                rotation='vertical', fontsize=8, color='white')
        ax.text(meals[:, 0], meals[:, 1] + meals[:, 2] / 2, f"fat\n{meals[0, 2]:.1f}", ha='center', va='center',
...
def figure_4a_poster(times, obs, meals, gpconv, gplfm, path):
    plt.rcParams.update({
        "font.family": "Times New Roman",  # use serif/main font for text elements
        "font.size": "16",
        "text.usetex": True,  # use inline math for ticks
    ...
    f, (a0, a2) = plt.subplots(2, 1, figsize=(8.0, 3.0), height_ratios=[2, 1], dpi=300, sharex=True)
    plt.xlim(48.0, times.max())
    f.tight_layout()
    ...
    os.makedirs(path, exist_ok=True)
    plt.savefig(path + 'figure_4a_poster.pdf', bbox_inches="tight")
    plt.close()


def figure_4a_poster1(times, obs, meals, gpconv, gplfm, path):
    plt.rcParams.update({
        "font.family": "Times New Roman",  # use serif/main font for text elements
        "font.size": "16",
        "text.usetex": True,  # use inline math for ticks
    ...
    f, (a1, a2) = plt.subplots(2, 1, figsize=(8.0, 4.0), height_ratios=[4.0, 1], dpi=300, sharex=True)
    plt.xlim(48.0, times.max())
    f.tight_layout()
    ...
    os.makedirs(path, exist_ok=True)
    plt.savefig(path + 'figure_4a_1_poster.pdf', bbox_inches="tight")
    plt.close()


def figure_4b_poster(times, meals, gpconv, gplfm, path):
    plt.rcParams.update({
        "font.family": "Times New Roman",  # use serif/main font for text elements
        "font.size": "16",
        "legend.title_fontsize": "10",
    ...
    f, (a0) = plt.subplots(1, 1, figsize=(4.0, 3.0), dpi=300, sharex=True)
    f.text(0.0, 0.5, 'Meal intake (g)', va='center', rotation='vertical')
    f.text(1.0, 0.5, 'Glucose response (mmol/l)', va='center', rotation='vertical')
    f.tight_layout()
    ...
    os.makedirs(path, exist_ok=True)
    plt.savefig(path + 'figure_4b_poster.pdf', bbox_inches="tight")
    plt.close()


def figure_4b_poster1(times, meals, gpconv, gplfm, path):
    plt.rcParams.update({
        "font.family": "Times New Roman",  # use serif/main font for text elements
        "font.size": "16",
        "legend.title_fontsize": "10",
    ...
    f, (a1) = plt.subplots(1, 1, figsize=(4.0, 3.0), dpi=300, sharex=True)
    f.text(0.0, 0.5, 'Meal intake (g)', va='center', rotation='vertical')
    f.text(1.0, 0.5, 'Glucose response (mmol/l)', va='center', rotation='vertical')
    f.tight_layout()
    ...
    os.makedirs(path, exist_ok=True)
    plt.savefig(path + 'figure_4b_1_poster.pdf', bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    # Plot Figure 4a for poster
    figure_4a_poster(times, obs, meals, gpconv_data, gplfm_data, path_save)

    # Plot Figure 4a_1 for poster
    figure_4a_poster1(times, obs, meals, gpconv_data, gplfm_data, path_save)
    ...
    # Plot Figure 4b for poster
    figure_4b_poster(times, meals, gpconv_data, gplfm_data, path_save)

    # Plot Figure 4b_1 for poster
    figure_4b_poster1(times, meals, gpconv_data, gplfm_data, path_save)

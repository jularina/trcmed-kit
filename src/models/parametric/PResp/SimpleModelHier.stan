...
real alpha2; // hyper-prior for alphas for fat
real alpha2_p[P];  // alphas for each patient for fat
...
for (p in 1:P){ 
  ...
  alpha2_p[p] ~ normal(alpha2,0.166);
  ...
  for (n in 1:N[p]){  
    ...
    for (m in 1:M[p]) { 
      if (fabs(tx[Mcumsum[p]+m]-t[p,n])<4){
        ...
        meals[m] = ((x1[Mcumsum[p]+m]*beta1_p[p])*exp(-0.5*(t[p,n]-tx_star[Mcumsum[p]+m]-3*(alpha1_p[p]))^2/(alpha1_p[p])^2))  +  ((x2[Mcumsum[p]+m]*beta2_p[p])*exp(-0.5*(t[p,n]-tx_star[Mcumsum[p]+m]-3*(alpha2_p[p]))^2/(alpha2_p[p])^2)); 
  ...
  for (n in 1:N[p]) {
    for (m in 1:M[p]) {
      if (fabs(tx[Mcumsum[p]+m]-t[p,n])<4){
        ...
        resp2[p,n,m] = (x2[Mcumsum[p]+m]*beta2_p[p])*exp(-0.5*(t[p,n]-tx_star[Mcumsum[p]+m]-3*(alpha2_p[p]))^2/(alpha2_p[p])^2);

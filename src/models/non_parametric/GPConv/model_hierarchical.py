...
from src.models.non_parametric.GPConv.kernels import HierarchicalCoregion, LFMKernel
import numpy as np
...
class HierarchicalModel(gpf.models.GPR):
   ...
   """
    ...
    def __init__(self, data, T, baseline_kernels, mean_functions, l, noise_variance=1.0,
        ...
        self.treatment_kernel_cor_meal1 = HierarchicalCoregion(output_dim=tf.shape(self.meals_sep)[0], rank=2,
                                                         num_patients=self.N,
                                                         meals_patient_idx=self.meals_patient_idx,
                                                         beta1=0.5, beta11=0.5, sigma_raw=0.1,
                                                         active_dims=[1, 2, 3])
    ...
    def create_fitting_data(self, x, meals_times, meals_vals, x_new=None, meals_times_new=None, meals_vals_new=None):
       ...
       """
        ...
        meals_vals_new *= tf.random.normal(tf.shape(meals_vals_new), mean=1.0, stddev=0.001,
    ...
    def K_total(self, x, deltas, baseline_scatter_ids, treatment_scatter_ids, x2=None, deltas2=None, action='total'):
    ...
    def K_treatment(self, x, d, scatter_ids, x2=None, d2=None):
        ...
        return K_scatter_meal1

    def K_treatment_diag(self, x, d, scatter_ids):
        ...
        return K_scatter_diag_meal1

    def predict_train(self, x, meals, patients_meals_idx):
        ...
        """
        f_mean_zero_meal1, f_var_meal1 = self.predict_multiple(x, meals, pm_idx=patients_meals_idx, full_cov=False)
        mnew = tf.concat([mf(Xi) for mf, Xi in zip(self.mean_functions, x)], axis=0)


        return f_mean_zero_meal1 + mnew, f_var_meal1

    def predict_multiple(self, x, meals, pm_idx, full_cov: bool = False):
        ...
        kmn_meal1 = self.K_treatment(self.x_sep, deltas, scatter_ids_2d_new, x_sep_new, deltas_new)
        ...
        if full_cov:
            _, scatter_ids_2d_new, _ = self.create_fitting_data(x_sep_new, tf.reshape(meals_sep_new[:, 0], (-1, 1)),
                                                                meals_sep_new[:, 1:])
            knn_meal1 = self.K_treatment(x_sep_new, deltas_new, scatter_ids_2d_new)
        else:
            knn_meal1 = self.K_treatment_diag(x_sep_new, deltas_new, scatter_ids_1d_new)
        ...
        return f_mean_zero_meal1, f_var_meal1


    def predict_baseline(self, x, full_cov: bool = False):
        """ Calculates predictions for all glucose data, passed at once.

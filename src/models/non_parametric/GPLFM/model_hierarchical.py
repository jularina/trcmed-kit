...
from src.models.non_parametric.GPLFM.kernels import HierarchicalCoregion, LFMLatentKernel, LFMKernelToy, HierarchicalCoregion1, LFMOutputKernelToy, TimeMarkedSquaredExponential
import numpy as np
...
from src.plot.non_parametric.GPLFM.plot_gp_kernels import plot_treatment_kernel, plot_baseline_kernel, plot_total_kernel, plot_treatment_kernel_train, plot_baseline_kernel_train, plot_tlse_kernel, plot_B_kernel, plot_total_kernel_whole_train, plot_total_kernel_whole_test
   ...
   """
    ...
    def create_fitting_data(self, x, meals_times, meals_vals, x_new=None, meals_times_new=None, meals_vals_new=None):
       ...
       """
        ...
        glucose_times_suitable = tf.gather(x_new, times_coord_new)
        meals_times_coord_new = tf.experimental.numpy.nonzero(tf.transpose(mask_new))[0]
        meals_times_suitable = tf.gather(meals_times_new, meals_times_coord_new)
        ...
        deltas = tf.concat(
            [tf.reshape(delta_times_suitable_new, (-1, 1)), tf.reshape(delta_times_suitable_new_idx, (-1, 1)),
             meals_vals_new_pert, glucose_times_suitable, meals_times_suitable], axis=1)
    ...
    def K_treatment_diag(self, x, d, scatter_ids, x_list, time='train'):
    ...
    def predict_train(self, x, meals, patients_meals_idx, time='train'):
        ...
        """
        f_mean_zero_meal1, f_mean_zero_meal2, f_var_meal1, f_var_meal2 = self.predict_multiple(x, meals, pm_idx=patients_meals_idx, full_cov=False, time=time)
    ...
    def predict_latent_mean_and_variance(self, x, meals, pm_idx, full_cov: bool = False):
        x_sep_new, meals_sep_new = self.prepare_treatment_input(x, meals)
        if len(meals_sep_new) == 0 or len(self.meals_sep) == 0:
            return tf.zeros_like(x_sep_new), tf.zeros_like(meals_sep_new)
        ...
        kmm = self.K_total(X, deltas, baseline_scatter_ids=self.block_diag_scatter_ids_train,
                           treatment_scatter_ids=scatter_ids_2d, action='total')
        kmm_stretched_noisy = self.add_noise_cov(kmm)
        ...
        self.treatment_kernel_cor_meal2.set_meals_pidx(pm_idx)
        self.treatment_kernel_cor_meal2.set_meals_pidx2(pm_idx)
        knn_meal1_kernel = gpf.kernels.SquaredExponential(variance=1.0, lengthscales=self.treatment1_base_kernel.l, active_dims=[0])
        knn_meal2_kernel = gpf.kernels.SquaredExponential(variance=1.0, lengthscales=self.treatment2_base_kernel.l, active_dims=[0])
        knn_meal1 = knn_meal1_kernel(deltas_new)
        knn_meal1 = tf.linalg.diag_part(knn_meal1)
        knn_meal1 = knn_meal1*self.treatment_kernel_cor_meal1(deltas_new, full_cov=False)
        knn_scatter_diag_meal1 = tf.reshape(tf.zeros_like(x_sep_new), (-1,))
        knn_scatter_diag_meal1 = tf.tensor_scatter_nd_add(knn_scatter_diag_meal1, scatter_ids_1d_new, knn_meal1)
        knn_meal2 = knn_meal2_kernel(deltas_new)
        knn_meal2 = tf.linalg.diag_part(knn_meal2)
        knn_meal2 = knn_meal2 * self.treatment_kernel_cor_meal2(deltas_new, full_cov=False)
        knn_scatter_diag_meal2 = tf.reshape(tf.zeros_like(x_sep_new), (-1,))
        knn_scatter_diag_meal2 = tf.tensor_scatter_nd_add(knn_scatter_diag_meal2, scatter_ids_1d_new, knn_meal2)
        ...
        self.treatment_kernel_cor_meal2.set_meals_pidx(self.meals_patient_idx)
        self.treatment_kernel_cor_meal2.set_meals_pidx2(pm_idx)
        klfm_meal1 = LFMLatentKernel(decay=self.treatment1_base_kernel.d, sensitivity=self.treatment1_base_kernel.s, lengthscales=self.treatment1_base_kernel.l, active_dims=[0])
        klfm_meal2 = LFMLatentKernel(decay=self.treatment2_base_kernel.d, sensitivity=self.treatment2_base_kernel.s, lengthscales=self.treatment2_base_kernel.l, active_dims=[0])
        kmn_meal1 = klfm_meal1(deltas, deltas_new)
        kmn_meal1 = kmn_meal1 * self.treatment_kernel_cor_meal1(deltas, deltas_new)
        kmn_scatter_meal1 = tf.matmul(tf.zeros_like(self.x_sep), tf.zeros_like(x_sep_new), transpose_b=True)
        kmn_scatter_meal1 = tf.tensor_scatter_nd_add(kmn_scatter_meal1, scatter_ids_2d_new, tf.reshape(kmn_meal1, [-1]))
        kmn_meal2 = klfm_meal2(deltas, deltas_new)
        kmn_meal2 = kmn_meal2 * self.treatment_kernel_cor_meal2(deltas, deltas_new)
        kmn_scatter_meal2 = tf.matmul(tf.zeros_like(self.x_sep), tf.zeros_like(x_sep_new), transpose_b=True)
        kmn_scatter_meal2 = tf.tensor_scatter_nd_add(kmn_scatter_meal2, scatter_ids_2d_new, tf.reshape(kmn_meal2, [-1]))
        ...
        err = ys - glucose_means
        conditional = gpf.conditionals.base_conditional
        f_mean_zero_meal1_latent, f_var_meal1_latent = conditional(
            kmn_scatter_meal1, kmm_stretched_noisy, knn_scatter_diag_meal1, err, full_cov=full_cov, white=False
        )
        f_mean_zero_meal2_latent, f_var_meal2_latent = conditional(
            kmn_scatter_meal2, kmm_stretched_noisy, knn_scatter_diag_meal2, err, full_cov=full_cov, white=False
        )
        ...
        self.treatment_kernel_cor_meal2.set_meals_pidx(pm_idx)
        self.treatment_kernel_cor_meal2.set_meals_pidx2(pm_idx)
        knn_meal1, knn_meal2 = self.K_treatment_diag(x_sep_new, deltas_new, scatter_ids_1d_new, x)
        ...
        conditional = gpf.conditionals.base_conditional
        f_mean_zero_meal1, f_var_meal1 = conditional(
            kmn_meal1, kmm_stretched_noisy, knn_meal1, err, full_cov=full_cov, white=False
        )
        f_mean_zero_meal2, f_var_meal2 = conditional(
            kmn_meal2, kmm_stretched_noisy, knn_meal2, err, full_cov=full_cov, white=False
        )

        return f_mean_zero_meal1_latent, f_mean_zero_meal2_latent, f_var_meal1_latent, f_var_meal2_latent, f_mean_zero_meal1, f_mean_zero_meal2, f_var_meal1, f_var_meal2

    # def predict_latent_mean_and_variance(self, x, meals, full_cov: bool = False):
    #     x_sep_new, meals_sep_new = self.prepare_treatment_input(x, meals)
    #     if len(meals_sep_new) == 0 or len(self.meals_sep) == 0:
    #         return tf.zeros_like(x_sep_new), tf.zeros_like(meals_sep_new)
    #
    ...
    #     X, Y, M = self.data
    #     deltas, scatter_ids_2d, _ = self.fitting_data
    #     # kmm_meal1 = self.treatment1_base_kernel(deltas)
    #     # kmm_meal1_stretched = tf.zeros([self.block_diag_scatter_ids_train[-1,0]+1,self.block_diag_scatter_ids_train[-1,0]+1],dtype=tf.dtypes.float64)
    #     # kmm_meal1_stretched = tf.tensor_scatter_nd_add(kmm_meal1_stretched, scatter_ids_2d, tf.reshape(kmm_meal1, [-1]))
    #     # kmm_meal1_stretched_noisy = self.add_noise_cov(kmm_meal1_stretched)
    #     # kmm_meal2 = self.treatment2_base_kernel(deltas)
    #     # kmm_meal2_stretched = tf.zeros([self.block_diag_scatter_ids_train[-1,0]+1,self.block_diag_scatter_ids_train[-1,0]+1],dtype=tf.dtypes.float64)
    #     # kmm_meal2_stretched = tf.tensor_scatter_nd_add(kmm_meal2_stretched, scatter_ids_2d, tf.reshape(kmm_meal2, [-1]))
    #     # kmm_meal2_stretched_noisy = self.add_noise_cov(kmm_meal2_stretched)
    #
    #     # kmm_meal1 = self.treatment1_base_kernel(deltas)
    #     # kmm_meal2 = self.treatment2_base_kernel(deltas)
    #     # kmm_stretched = tf.zeros(
    #     #     [self.block_diag_scatter_ids_train[-1, 0] + 1, self.block_diag_scatter_ids_train[-1, 0] + 1],
    #     #     dtype=tf.dtypes.float64)
    #     # kmm_stretched = tf.tensor_scatter_nd_add(kmm_stretched, scatter_ids_2d, tf.reshape(kmm_meal1, [-1]))
    #     # kmm_stretched = tf.tensor_scatter_nd_add(kmm_stretched, scatter_ids_2d, tf.reshape(kmm_meal2, [-1]))
    #     # kmm_stretched_noisy = self.add_noise_cov(kmm_stretched)
    #
    #     kmm = self.K_total(X, deltas, baseline_scatter_ids=self.block_diag_scatter_ids_train,
    #                        treatment_scatter_ids=scatter_ids_2d, action='total')
    #     kmm_stretched_noisy = self.add_noise_cov(kmm)
    ...
    #     ########## REAL CASE ##############
    #     # Knn kernel
    #     knn_meal1 = self.treatment1_base_kernel(deltas_new)
    #     knn_meal1 = tf.linalg.diag_part(knn_meal1)
    #     knn_scatter_diag_meal1 = tf.reshape(tf.zeros_like(x_sep_new), (-1,))
    #     knn_scatter_diag_meal1 = tf.tensor_scatter_nd_add(knn_scatter_diag_meal1, scatter_ids_1d_new, knn_meal1)
    #     knn_meal2 = self.treatment2_base_kernel(deltas_new)
    #     knn_meal2 = tf.linalg.diag_part(knn_meal2)
    #     knn_scatter_diag_meal2 = tf.reshape(tf.zeros_like(x_sep_new), (-1,))
    #     knn_scatter_diag_meal2 = tf.tensor_scatter_nd_add(knn_scatter_diag_meal2, scatter_ids_1d_new, knn_meal2)
    #
    #     # Kmn kernel
    #     kmn_meal1 = self.treatment1_base_kernel(deltas, deltas_new)
    #     kmn_scatter_meal1 = tf.matmul(tf.zeros_like(self.x_sep), tf.zeros_like(x_sep_new), transpose_b=True)
    #     kmn_scatter_meal1 = tf.tensor_scatter_nd_add(kmn_scatter_meal1, scatter_ids_2d_new, tf.reshape(kmn_meal1, [-1]))
    #     kmn_meal2 = self.treatment2_base_kernel(deltas, deltas_new)
    #     kmn_scatter_meal2 = tf.matmul(tf.zeros_like(self.x_sep), tf.zeros_like(x_sep_new), transpose_b=True)
    #     kmn_scatter_meal2 = tf.tensor_scatter_nd_add(kmn_scatter_meal2, scatter_ids_2d_new, tf.reshape(kmn_meal2, [-1]))
    #
    #     # Compute conditionals
    ...
    def predict_baseline(self, x, full_cov: bool = False, time='train'):
        ...
        self.treatment_kernel_cor_meal2.set_meals_pidx(self.meals_patient_idx)
        self.treatment_kernel_cor_meal2.set_meals_pidx2(self.meals_patient_idx)
        kmm = self.K_total(X, deltas, baseline_scatter_ids=self.block_diag_scatter_ids_train, treatment_scatter_ids=scatter_ids_2d, action='total')
        kmm_noisy = self.add_noise_cov(kmm)
